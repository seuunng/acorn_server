...
public class RegionGroupService {
    ...
    public Map<String, Map<String, Long>> countCustomersByRegionGroup() {
        ...
        Map<String, Long> regionGroupProvinceCounts = regionGroups.stream()
        		 .filter(region -> Objects.nonNull(region.getRegiongroupProvince())) // null 값 필터링
                 .collect(Collectors.groupingBy(region -> Objects.requireNonNullElse(region.getRegiongroupProvince(), "UNKNOWN"), Collectors.counting()));
        Map<String, Long> regionGroupCityCounts = regionGroups.stream()
        		 .filter(region -> Objects.nonNull(region.getRegiongroupCity())) // null 값 필터링
                 .collect(Collectors.groupingBy(region -> Objects.requireNonNullElse(region.getRegiongroupCity(), "UNKNOWN"), Collectors.counting()));
        Map<String, Long> regionGroupTownCounts = regionGroups.stream()
        		.filter(region -> Objects.nonNull(region.getRegiongroupTown())) // null 값 필터링
                .collect(Collectors.groupingBy(region -> Objects.requireNonNullElse(region.getRegiongroupTown(), "UNKNOWN"), Collectors.counting()));
